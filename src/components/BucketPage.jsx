...
export function readResults(bucketKey) {
  try {
...
export function narrativeOnly(md) {
  return md
    .split(/\n+---\n+/)
    .filter(seg => {
      const s = seg.trim()
    ...
    <section style={{ margin: '48px 0 8px' }}>
      <div style={{ background: `linear-gradient(160deg, ${T.forest} 0%, ${T.forest700} 100%)`, borderRadius: 20, padding: 'clamp(32px,5vw,52px)', textAlign: 'center', color: '#fff', boxShadow: T.shadow }}>
        ...
        <p style={{ fontSize: 16, lineHeight: 1.7, color: 'rgba(255,255,255,.75)', maxWidth: 540, margin: '30px auto 0' }}>
          ...
          <Link
            href={`/apply?src=quiz_${bucketKey}`}
...
function DocPage({ meta }) {
  /* Read after mount, never during render. This page is prerendered to static
     HTML at build time, where there is no sessionStorage — so a useState
     initialiser that reads it makes the first client render disagree with the
     served HTML, and React discards the whole tree and rebuilds it. That fired
     for every visitor arriving from the quiz. */
  const [results, setResults] = useState(null)
  // The extra render this causes is the point: the first one has to match the
  // prerendered HTML, and only the second can know what is in storage.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { setResults(readResults(meta.bucketKey)) }, [meta.bucketKey])
